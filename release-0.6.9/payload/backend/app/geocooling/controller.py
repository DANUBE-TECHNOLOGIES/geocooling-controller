...
from app.geocooling.thermal import ThermalEstimator
...
class GeoCoolingController:
    ...
    def __init__(self) -> None:
        ...
        self._last_safety_decision: SafetyDecision = self.safety_manager.evaluate(
            indoor_temperature_c=self._thermal_snapshot["indoor_temperature_c"],
            indoor_humidity_percent=self._thermal_snapshot["indoor_humidity_percent"],
            surface_temperature_c=self._thermal_snapshot["surface_temperature_c"],
        )
    ...
    def update_thermal_snapshot(
        ...
        **values: float | None,
        ...
        with self._lock:
            for key in self._thermal_snapshot:
                if key in values:
                    self._thermal_snapshot[key] = values[key]
            self._last_safety_decision = self.safety_manager.evaluate(
                indoor_temperature_c=self._thermal_snapshot["indoor_temperature_c"],
                indoor_humidity_percent=self._thermal_snapshot["indoor_humidity_percent"],
                surface_temperature_c=self._thermal_snapshot["surface_temperature_c"],
            )
            thermal = self.thermal_estimator.update(**self._thermal_snapshot)
            return {
                "snapshot": dict(self._thermal_snapshot),
                "safety": self._last_safety_decision.as_dict(),
                "thermal": thermal,
            }

    def thermal_status(self) -> dict[str, Any]:
        return self.thermal_estimator.status()
