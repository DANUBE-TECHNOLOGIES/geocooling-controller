...
log "Installation des fichiers 0.6.9"
rm -rf "$TARGET/backend" "$TARGET/tests"
...
cp -a "$PAYLOAD/CHANGELOG-0.6.9.md" "$TARGET/CHANGELOG-0.6.9.md"
...
docker compose exec -T backend python -c \
  'from app.geocooling.runtime import RuntimeGuards; from app.geocooling.safety import GeoCoolingSafetyManager; from app.geocooling.api import router; print("GeoCooling 0.6.9 OK")'
...
for path in ("/health/live", "/geocooling/status", "/geocooling/diagnostics", "/geocooling/safety", "/geocooling/thermal"):
...
trap - EXIT
log "Installation 0.6.9 terminée"
