...
log "Installation des fichiers 0.6.7"
rm -rf "$TARGET/backend" "$TARGET/tests"
...
cp -a "$PAYLOAD/CHANGELOG-0.6.7.md" "$TARGET/CHANGELOG-0.6.7.md"

log "Validation de la syntaxe Python"
docker run --rm -v "$TARGET/backend:/app/backend:ro" python:3.13-alpine \
...
docker compose exec -T backend python -c \
  'from app.geocooling.safety import GeoCoolingSafetyManager; from app.geocooling.api import router; print("GeoCooling 0.6.7 OK")'
...
trap - EXIT
log "Installation 0.6.7 terminée"
