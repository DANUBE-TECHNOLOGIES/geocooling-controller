...
DATA_ROOT = REPO / "rc3-prediction-journal"
...
INTERVAL_SECONDS = max(
    ...
    int(
        os.getenv(
            "GEOCOOLING_RC37_INTERVAL_SECONDS",
            "300",
        )
    ),
)
...
def get_json(route: str) -> dict[str, Any]:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "GeoCooling-RC37/1.0",
        },
    )
...
def main() -> int:
    ...
    metadata = {
        "schema": "geocooling.rc37.prediction-journal.v1",
        "started_at": now_iso(),
        "interval_seconds": INTERVAL_SECONDS,
        "prediction_route": (
            "/geocooling/rc3/weather-inertia/live"
        ),
        "context_route": (
    ...
    snapshots_file = session / "prediction-snapshots.jsonl"
    ...
    try:
        while running and not STOP_FILE.exists():
            ...
            try:
                ...
                snapshot = {
                    "schema": (
                        "geocooling.rc37.prediction-snapshot.v1"
                    ),
                    "captured_at": now_iso(),
