...
from app.automation.api import (
    configure_automation_router,
    router as automation_router,
)
from app.automation.service import AutomationService
from app.rules.api import (
    configure_rules_router,
    router as rules_router,
)
from app.rules.service import RuleEngineService

APP_VERSION = os.getenv("APP_VERSION", "0.6.7")
...
def create_thermal_snapshot() -> None:
    with engine.begin() as connection:
        ...
        # Alimentation directe du moteur thermique GeoCooling.
        #
        # Les températures spécifiques peuvent être associées à des
        # capteurs normalisés au moyen des variables d'environnement :
        #
        # GEOCOOLING_SURFACE_SENSOR
        # GEOCOOLING_FLOOR_SUPPLY_SENSOR
        # GEOCOOLING_FLOOR_RETURN_SENSOR
        # GEOCOOLING_SOURCE_INLET_SENSOR
        # GEOCOOLING_SOURCE_OUTLET_SENSOR
        # GEOCOOLING_FLOW_SENSOR
        try:
            from app.geocooling.api import (
            ...
            driver_status = geocooling_controller.driver.status()

            geocooling_payload: dict[str, Any] = {
                "timestamp": utc_now().isoformat(),
                "indoor_temperature_c": indoor_temperature,
                "indoor_humidity_percent": indoor_humidity,
                "surface_temperature_c": optional_measurement(
                    "GEOCOOLING_SURFACE_SENSOR",
                    "temperature",
                ),
                "floor_supply_temperature_c": optional_measurement(
                    "GEOCOOLING_FLOOR_SUPPLY_SENSOR",
                    "temperature",
                ),
                "floor_return_temperature_c": optional_measurement(
                    "GEOCOOLING_FLOOR_RETURN_SENSOR",
                    "temperature",
                ),
                "source_inlet_temperature_c": optional_measurement(
                    "GEOCOOLING_SOURCE_INLET_SENSOR",
                    "temperature",
                ),
                "source_outlet_temperature_c": optional_measurement(
                    "GEOCOOLING_SOURCE_OUTLET_SENSOR",
                    "temperature",
                ),
                "outdoor_temperature_c": outdoor_temperature,
                "flow_rate_l_min": optional_measurement(
                    "GEOCOOLING_FLOW_SENSOR",
                    "flow",
                ),
                "pump_running": bool(
                    driver_status.get("pump_running", False)
                ),
                "valve_open": bool(
                    driver_status.get("valve_open", False)
                ),
            }

            geocooling_payload = {
                key: value
                for key, value in geocooling_payload.items()
                if value is not None
            }

            geocooling_controller.ingest_thermal_snapshot(
                geocooling_payload
            )

        except Exception:
            # Une erreur du nouveau moteur ne doit jamais interrompre
            # l'historisation thermique principale.
            logger.exception(
                "Erreur d'alimentation du moteur GeoCooling"
            )
...
rule_engine_service = RuleEngineService(engine=engine)
configure_rules_router(rule_engine_service)


@asynccontextmanager
async def lifespan(app: FastAPI):
    initialize_database()
    rule_engine_service.initialize()
    event_bus.start()
    collector.start()
...
def health() -> JSONResponse:
    ...
    return JSONResponse(
        content=jsonable_encoder({
            ...
            "automation": automation_service.diagnostics(),
            "rules": rule_engine_service.diagnostics(),
...
automation_service = AutomationService(
    ...
    geocooling_controller=geocooling_controller,
)
configure_automation_router(automation_service)
