...
class GeoCoolingEventBus:
    ...
    # PATCH C013.0R2 — Subscription Manager Dispatch
    def _c0130r2_resolve_event_type(
        self,
        args,
        kwargs,
    ):
        """
        Retrouve event_type sans dépendre de la signature
        concrète de publish().
        """
        ...
        try:
            signature = inspect.signature(
                self._c0130r2_original_publish
            )
            ...
            if "event_type" in bound.arguments:
                return bound.arguments.get(
                    "event_type"
                )
        except Exception:
            pass
    ...
    def _c0130r2_dispatch_event(
        ...
        """
        Distribue l'événement publié au gestionnaire.

        Toute erreur est neutralisée afin de ne jamais
        perturber le comportement historique du bus.
        """
        ...
        try:
            ...
            event_type = (
                self._c0130r2_resolve_event_type(
                    args,
                    kwargs,
                )
            )
    ...
    @functools.wraps(_c0130r2_original_publish)
    def publish(self, *args, **kwargs):
        """
        Exécute la publication historique puis distribue
        l'événement aux abonnés.
        """

        result = self._c0130r2_original_publish(
            *args,
            **kwargs,
        )

        self._c0130r2_dispatch_event(
            result,
            args,
            kwargs,
        )
